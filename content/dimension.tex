...
Let $V$ be a finite-dimensional vector space over a field $K$. The following structural principles hold:
\begin{enumerate}
...
\label{prop:12.2}
Let $V$ be a finite-dimensional vector space over $K$. Then every linear subspace $U \subseteq V$ is also finite-dimensional. Moreover, $\dim U \le \dim V$, with the equality $\dim U = \dim V$ holding if and only if $U = V$.
\end{proposition}
...
Furthermore, if $\dim U = \dim V = n$, then $U$ contains a linearly independent list of length $n$. By Theorem \ref{thm:12.1}, such a list satisfies property \textbf{(a)} and is therefore also a basis for $V$. This implies $\Sp(v_1, \dots, v_n) = V$. Since $U = \Sp(v_1, \dots, v_n)$ by the construction of the basis, we conclude $U = V$.
\end{proof}