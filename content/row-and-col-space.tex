...
\begin{proof}
By Proposition \ref{prop:row_space_invariance}, $\RowS(A) = \RowS(B)$. Let $r$ be the number of pivots in $B$. The non-zero rows of $B$ are $w'_1, \dots, w'_r$. By definition, they span $\RowS(B)$. 
...
\begin{proof}
By Theorem \ref{thm:row_basis}, the non-zero rows of any row-reduced echelon form $B$ of $A$ form a basis for $\RowS(A)$. The number of such rows is exactly the number of pivots. Since the dimension of a vector space is a fixed property, the number of pivots must be the same regardless of the specific row-reduction process used to reach an echelon form.
\end{proof}
