...
By definition, $w \in \Sp(w_{1}, \dots, w_{n})$ if and only if there exist scalars $x_{1}, \dots, x_{n} \in K$ such that $w = x_{1} w_{1} + \dots + x_{n} w_{n}$. We can assemble the vectors $w_{j}$ as columns of an $m \times n$ matrix $A$. This abstract combination is equivalent to the matrix equation:
\[
...
\]
Thus, $w$ is in the span iff the system $Ax = w$ is consistent. We analyze this using Gauss elimination on the augmented matrix $(A \mid w)$. To describe the span precisely, we take a general vector $b = (b_{1}, \dots, b_{m})^{T}$ and reduce the augmented matrix to a row-reduced echelon form $(A' \mid b')$. 

As shown in Prof. Biran's notes, the resulting matrix has the following characteristic block structure:
\begin{equation}
...
\right)
\end{equation}
The system is consistent if and only if the entries in $b'$ corresponding to the zero-rows of $A'$ vanish. Therefore:
\[
...
\label{ex:span_calc}
Let $v_{1} = (1, 1, 1)^{T}$ and $v_{2} = (1, 0, -1)^{T} \in K^{3}$. To describe $\Sp(v_{1}, v_{2})$, we row-reduce $(A \mid b)$:
\[
...
\]
Thus, $\Sp(v_{1}, v_{2}) = \{ (b_{1}, b_{2}, b_{3})^{T} \in K^{3} \mid b_{1} - 2b_{2} + b_{3} = 0 \}$.
\end{example}

\begin{answer}[To Question 3]
The vectors are linearly independent if $Ax = 0$ has only the trivial solution. This occurs if and only if the row-reduced echelon form $A'$ has a pivot in \qt{every column}. If a column lacks a pivot, the corresponding variable is \qt{free}, allowing for non-trivial solutions and thus linear dependence.
\end{answer}
...
\begin{proof}
\begin{enumerate}
    \item Let $A = (v_{1} \mid \dots \mid v_{n})$. The number of pivots in $A'$ is at most the number of rows $m$. If $n > m$, at least $n - m$ columns are without pivots. These free variables imply non-trivial solutions to $Ax = 0$, hence dependence.
    \item The number of pivots is at most $n$. If $n < m$, there are at least $m - n$ zero-rows in $A'$. By \eqref{eq:block_consistency}, there exist vectors $b$ such that the system is inconsistent.
\end{enumerate}
\end{proof}
...
\begin{enumerate}
    \item The \qt{column space} $\operatorname{Cols}(A) \subseteq K^{m}$ is the span of the columns of $A$.
    \item The \qt{row space} $\operatorname{Rows}(A) \subseteq K^{n}$ is the span of the rows of $A$.
\end{enumerate}
\end{definition}
...
Elementary row operations do not change the row space: $\operatorname{Rows}(A) = \operatorname{Rows}(A')$.
\end{proposition}

\begin{proof}
New rows are linear combinations of old rows, and operations are reversible.
\end{proof}
...
\begin{proof}
By Proposition \ref{prop:preservation_relations}, row operations preserve the linear dependence relations between columns. Thus, a set of columns in $A$ is linearly independent (or spans the column space) if and only if the corresponding set of columns in $B$ possesses the same property in $\operatorname{Cols}(B)$.
...
\[
K^r \times \{0\}^{m-r} = \Sp(e_1, \dots, e_r) \subseteq \operatorname{Cols}(B) \subseteq K^r \times \{0\}^{m-r}.
\]
This double inclusion forces the equality $\operatorname{Cols}(B) = \Sp(e_1, \dots, e_r) = K^r \times \{0\}^{m-r}$. Consequently, the pivot columns of $B$ form a basis for its column space. By the preservation of linear relations, the corresponding columns in the original matrix $A$ form a basis for $\operatorname{Cols}(A)$.
\end{proof}
...
\label{thm:row_basis}
The non-zero rows of the row-reduced echelon matrix $B$ form a basis for $\operatorname{Rows}(A)$.
\end{theorem}

\begin{corollary}[Rank Invariance]
\label{cor:rank_invariance}
The number of pivots is independent of the sequence of row operations, as it always equals $\dim \operatorname{Rows}(A)$.
\end{corollary}

% thm:13.13
\begin{theorem}[The Rank Equality]
\label{thm:rank_equality}
For any $A \in \M_{m \times n}(K)$, $\dim \operatorname{Rows}(A) = \dim \operatorname{Cols}(A)$. This shared dimension is the \qt{rank} of $A$, denoted $\rank(A)$.
\end{theorem}

\begin{proof}
Let $r$ be the number of pivots in the RREF of $A$. By Theorem \ref{thm:col_basis}, $\dim \operatorname{Cols}(A) = r$. By Theorem \ref{thm:row_basis}, $\dim \operatorname{Rows}(A) = r$. Thus, the dimensions are equal.
\end{proof}
