...
Before we solve systems of linear equations, we must define the algebraic structures over which these equations are built. Throughout this course, we will work over a base \textit{field} (in German: \textit{Körper}), usually denoted by $K$. 
...
A linear equation in $n$ variables $x_1, \dots, x_n$ over a field $K$ is an equation of the form:
\[
a_1 x_1 + \dots + a_n x_n = b
\]
where $a_1, \dots, a_n \in K$ are the \textit{coefficients} and $b \in K$ is the \textit{constant term}.
\begin{itemize}
    \item If $b = 0$, the equation is called \textbf{homogeneous}.
    \item If $b \ne 0$, the equation is called \textbf{inhomogeneous}.
\end{itemize}
\end{definition}
...
A \textit{solution} to the system is a sequence of scalars $s_1, \dots, s_n \in K$ that simultaneously satisfies all $m$ equations when we substitute $x_j = s_j$.
\end{definition}

\begin{definition}[Equivalence]
Two systems of linear equations (in the same variables) are called \textbf{equivalent} if they have exactly the same set of solutions.
\end{definition}
...
An $m \times n$ \textit{matrix} over $K$ is a rectangular array consisting of $m \cdot n$ elements of $K$, arranged in $m$ rows and $n$ columns.
\end{definition}
...
\begin{enumerate}
    \item \textbf{Column Vectors:} An $n \times 1$ matrix is called a column vector. The set of all column vectors of length $n$ is denoted by $K^n := M_{n \times 1}(K)$.
    \[
    \begin{pmatrix} x_1 \\ x_2 \\ \vdots \\ x_n \end{pmatrix} \in K^n
    \]
    \item \textbf{Row Vectors:} A $1 \times n$ matrix is called a row vector. The set of all row vectors of length $n$ is denoted by $K_{\text{row}}^n := M_{1 \times n}(K)$.
    \[
...
A x = b
\end{equation}
where $A \in M_{m \times n}(K)$ is the \textit{coefficient matrix}, $x \in K^n$ is the vector of variables, and $b \in K^m$ is the constant vector.

\subsection{Elementary Row Operations (EROs)}

To solve $A x = b$, we define the \textit{augmented matrix} $(A|b) \in M_{m \times (n+1)}(K)$ by appending $b$ as an extra column to $A$. We can manipulate this matrix using three basic operations that correspond exactly to operations on the equations themselves.
