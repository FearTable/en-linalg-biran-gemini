...
\begin{theorem}[The Canonical Isomorphism from the Homomorphism Space $\Hom(V,W)$
   to the Matrix Space $\mathcal{M}_{m \times n}(K)$]
...
\end{proof}
\begin{proof}
  First, we verify that $(\End(V), +, \circ)$ satisfies the ring axioms. We already know that $(\End(V), +)$ is an abelian group from the proposition regarding the vector space structure of $\Hom(V,W)$. We now verify the properties related to multiplication (composition):
  \begin{enumerate}
  ...
  \begin{enumerate}
    \item \textbf{Preservation of Multiplication}: For any $T_1, T_2 \in \End(V)$, a fundamental result from Linear Algebra I states that the matrix representation of a composition of linear maps is the product of their matrix representations. That is,
    \[ \Psi_{\mathcal{B}}^{\mathcal{B}}(T_1 \circ T_2) = [T_1 \circ T_2]_{\mathcal{B}}^{\mathcal{B}} = [T_1]_{\mathcal{B}}^{\mathcal{B}} \cdot [T_2]_{\mathcal{B}}^{\mathcal{B}} = \Psi_{\mathcal{B}}^{\mathcal{B}}(T_1) \cdot \Psi_{\mathcal{B}}^{\mathcal{B}}(T_2). \]
    \item \textbf{Preservation of Multiplicative Identity}: The matrix representation of the identity endomorphism with respect to any basis $\mathcal{B}$ is the identity matrix $I_n$.
...
\begin{definition}[The Dual Space and Linear Functionals]
  Let $V$ be a vector space over $K$. The \qt{dual space} of $V$ is defined to be the vector space $V^* := \Hom(V,K)$. The elements of $V^*$ are linear maps $\ell \colon V \to K$, and are sometimes called \qt{functionals}.
\end{definition}
...
Let $V = K_{\text{col}}^n$. Then, we can identify $V^*$ with $K_{\text{row}}^n$ as follows: If $\ell \in K_{\text{row}}^n$, then $\ell$ defines a functional $f_{\ell} \colon K_{\text{col}}^n \to K$ by $f_{\ell}(v) := \ell \cdot v \in M_{1 \times 1}(K) \cong K$. The map $D \colon K_{\text{row}}^n \to (K_{\text{col}}^n)^*$ defined by $D(\ell) := f_{\ell}$ is a linear map, and therefore, it is an isomorphism.
...
\begin{lemma}[Construction of the Dual Basis]
  The elements $v_1^*, \dots, v_n^* \in V^*$ form a basis for $V^*$. This basis is called the \qt{dual basis} to $\mathcal{B}$. We denote this basis by $\mathcal{B}^* = (v_1^*, \dots, v_n^*)$. Moreover, for all $\ell \in V^*$, $\ell = \sum_{i=1}^n \ell(v_i) \cdot v_i^*$.
\end{lemma}
...
\begin{proof}
  ...
  Let $A := [\id_V]$ and write $A = (a_{ij})$. By definition, $v_i = \sum_{k=1}^n a_{ki} w_k$.
...
\begin{lemma}[Definition of the Dual Map]
  Let $V$ and $W$ be vector spaces over $K$, and let $T \colon V \to W$ be a linear map. Define a new map $T^* \colon W^* \to V^*$ by $T^*(\ell) := \ell \circ T$ for all $\ell \in W^*$. Then, $T^*$ is a linear map. It is called the \qt{dual map} to $T$.
\end{lemma}
...
\begin{summary}
In summary, applying $(-)^*$, or $\Hom(-,K)$, takes every vector space $V$ and gives us a new vector space $V^*$. It also takes any linear map $T \colon V \to W$ and gives us a new map $T^*$ between the duals of $V$ and $W$, but the arrow is reversed: $T^* \colon W^* \to V^*$. This is an example of a \qt{contravariant functor}.
\end{summary}

\begin{lemma}[Transposition of the Representation Matrix]
  Let $S \colon V \to W$ be a linear map between two finite-di\-men\-sional vector spaces $V$ and $W$ over $K$. Let $\mathcal{B}$ be a basis for $V$, and let $\mathcal{C}$ be a basis for $W$. Consider $S^* \colon W^* \to V^*$ and the bases $\mathcal{C}^*, \mathcal{B}^*$ for $W^*, V^*$, respectively. Then, $[S^*]_{\mathcal{B}^*}^{\mathcal{C}^*} = ([S]_{\mathcal{C}}^{\mathcal{B}})^T$.
\end{lemma}
...
Let $V$ be a vector space over $K$. We have $V^*$, but also $V^{**} := (V^*)^*$, obtained by applying the \qt{dual} operation one more time to $V^*$. $V^{**}$ is sometimes called the \qt{bidual space} of $V$. Is there any relation between $V^{**}$ and $V$?
...
\begin{proof}
  ...
  To finish the proof, note that $\dim(V^*)^* = \dim V^* = \dim V$. Therefore, since $\tau \colon V \to (V^*)^*$ is injective, it must be an isomorphism.
\end{proof}
...
\begin{definition*}[The Quotient $V/U$ And The Quotient Map $\pi$] The space $V/U$ is called the \qt{quotient} of $V$ by $U$, or $V$ modulo $U$. The map $\pi$ is called the \qt{quotient map}, or the \qt{projection onto the quotient space}.
\end{definition*}
...
\begin{itemize}
  ...
  \item If $U$ is a \textbf{line} through the origin, the equivalence classes $[v]$ forming the space $\mathbb{R}^2/U$ are the lines parallel to $U$, as seen in Figure \ref{fig:quotient_r2}.
\end{itemize}
...
Similarly, we can extend this intuition to $\mathbb{R}^3$, as illustrated in Figure \ref{fig:quotient_r3}:
\begin{itemize}
  ...
  \item If $U$ is a \textbf{plane} through the origin, then $\mathbb{R}^3/U$ is 1-dimensional (by Proposition \ref{prop:dim_quotient}) and therefore isomorphic to a line. Each element $[v]$ is a plane parallel to $U$. The quotient space can be viewed as the set of all such parallel planes \qt{stacked} along a transversal direction (see Figure \ref{fig:quotient_plane} and Proposition \ref{prop:iso_complement}).
  \item If $U$ is a \textbf{line} through the origin, then $\mathbb{R}^3/U$ is 2-dimensional (by Proposition \ref{prop:dim_quotient}) and therefore isomorphic to a plane. Each element $[v]$ is a line parallel to $U$ (see Figure \ref{fig:quotient_line} and Proposition \ref{prop:iso_complement}).
...
\begin{remark}
  ...
  Thus, the space of all such lines is 2-dimensional. This geometric intuition aligns perfectly with the dimension formula established in Proposition \ref{prop:dim_quotient}. In the case where $U = \{0\}$, the equivalence class is a point (0-dimensional), and we retain all 3 degrees of freedom of the ambient space.
\end{remark}
...
\begin{proposition}[Dimension of the Quotient Space] \label{prop:dim_quotient}
...
\begin{proposition}[Canonical Isomorphism from a Complement] \label{prop:iso_complement}
...
\begin{proposition}[Annihilators and Dual Quotients]
  Let $U \subseteq V$ be a subspace, and define the \qt{annihilator} $U^\perp := \{\ell \in V^* \mid \ell|_U = 0\}$. Then:
  \begin{enumerate}
